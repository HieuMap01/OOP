...
class Pet{
    ...
    protected: 
    void setAll(string  t,string  mt){
        this-> ten =t;
        this -> moitruong=mt;
    }
    public:
        virtual void print() const{
            cout<<"Ten : "<<ten<<endl<<"Moi truong : "<<moitruong<<endl;
        }
         virtual ~ Pet (){}
};
...
class Cat: public Pet{
    string  dodai;
    string  checkCat(string  moitruong) const{
...
int main(){
    try {
        Pet * r [2]={new Cat ("ABC","can","ngan"),new Fish(" ABC","nuoc","duc")};
        for(int i = 0;i<2;i++){
            r[i]->print();
        }
    ...
    try {
        Pet * r [2]={new Cat ("ABC","nuoc","ngan"),new Fish(" ABC","rung","duc")};
        for(int i = 0;i<2;i++){
            r[i]->print();
        }
